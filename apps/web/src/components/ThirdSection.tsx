import { Card, CardContent, CardTitle, ScrollReveal } from '@mono/ui'; 
    ...
    <section id="routine" className="py-24 px-6 max-w-7xl mx-auto">
      <ScrollReveal>
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-6xl font-bold mb-4">
            다람쥐의 하루는 <span className="text-primary">언제나</span> 분주해!
          </h2>
        </div>
      </ScrollReveal>
      ...
      <ScrollReveal delay={0.2}>
        <div className="mb-16">
          ...
          <p className="text-muted-foreground text-lg">매일매일 성실하게, 하루도 빠짐없이 달리고 있어요</p>
        </div>
      </ScrollReveal>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        {squirrelRoutine.map((item, index) => (
          <ScrollReveal key={index} delay={0.1 * index} direction="up">
            <Card className="transition-all duration-500 group hover:border-primary/60 hover:shadow-xl hover:shadow-primary/10 hover:scale-105 h-full">
              <CardContent className="p-7">
                <div className="text-primary text-sm font-bold mb-3">
                  {item.time}
                </div>
                <CardTitle className="text-xl md:text-2xl">{item.task}</CardTitle>
              </CardContent>
            </Card>
          </ScrollReveal>
        ))}
